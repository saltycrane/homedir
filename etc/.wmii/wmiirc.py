...
# Theme
background = '#000000'
floatbackground = '#000000'

wmii['font'] = '-*-fixed-medium-r-*-*-12-*-*-*-*-*-*-*'
wmii['normcolors'] = '#888888', '#000000', '#111111'
...
@defmonitor
def temp(self):
    temp = call('acpi', '-t').split(',')[1]
    temp = temp.split()[0]
    return ' Temp: %s ' % temp

@defmonitor
def free_mem(self):
    lines = call('free', '-m').splitlines()
    tot = lines[1].split()[1]
    used = lines[2].split()[2]
    return ' Mem: %.1f%% ' % (100 * float(used) / float(tot))

@defmonitor
def load(self):
    return ' Load: %s ' % re.sub(r'^.*: ', '', call('uptime')).replace(', ', ' ')

@defmonitor
def time(self):
    return call("date", "+%Y-%m-%d %l:%M:%S %p")
...
keys.bind('main', {
    # pre-defined column widths (eliot)
    '%(mod)s-z': lambda k: call('wmii.py', 'scw', '20', '80'),
    '%(mod)s-x': lambda k: call('wmii.py', 'scw', '40', '60'),
    '%(mod)s-c': lambda k: call('wmii.py', 'scw', '50', '50'),
    '%(mod)s-v': lambda k: call('wmii.py', 'scw', '60', '40'),
    '%(mod)s-b': lambda k: call('wmii.py', 'scw', '80', '20'),
